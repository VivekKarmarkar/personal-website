...
5. **AutoHeightIframe SUPERSEDES Pattern C's fixed-height doctrine for responsive embeds** (post blind-review round, commits f6ab1af + de85cca). Fixed per-breakpoint heights cut 291–491px on real phones and left 1.3–1.9k px dead tails at laptop split (combined Codex+relay review). `src/components/AutoHeightIframe.jsx`: same-origin measurement via **body `getBoundingClientRect().bottom` + 16** (NOT `documentElement.scrollHeight` — that floors at the iframe's own height, so it grows but never shrinks back), ResizeObserver on the inner doc, 2px dead-band against feedback loops (the d8ac629 failure), plus a capped 500ms re-attach poll because until the inner `load` fires (slow subresources — VOYP's YouTube embed) the observer can bind to the interstitial about:blank. Verified 31/31 live across 5 regimes incl. grow-AND-shrink on resize. Fixed heights remain fine for non-responsive embeds (the 17,500px data portrait is untouched). Same round: inner `min-width: 320px` dropped from the lean apps (64px unreachable at 320-wide phones), actions grid 3-across, and the NNLego Raw-PyTorch missing-comma fix (one char, ast.parse-verified live). Review-loop methodology now standard: phone checks REQUIRE touch emulation (`pointer: coarse` must match — Codex's banner complaint was exactly this artifact), and live fetches are verified by content, never status (soft-200 fallback).
...
### Native rebuild round — 2026-09-02 — Pattern A ×4 (Vivek's order; iframes retired)
ClaudeSense (`12576ba` + flow-chart fix `79a861b`), Kiwi (`123a084`), VOYP (`f1693a0`), Reddit (`48843a1`) rebuilt natively via /port-reference-site after the Pattern C iframe versions were judged the wrong pattern for audience-facing lines. **Rules learned:** (1) Pattern C is not for D-line/life-layer content pages — those are native, full stop. (2) **Figure fidelity is a hard rule** (Vivek: "Whenever good things were there, clear diagrams, clean explanations, do not fuck with it"): render the SOURCE headlessly, element-screenshot every figure (diagram, flow chart, table, video panel, hero frame, cards) at 1280 and 390, rebuild with the source's OWN CSS values (colors, radii, padding, chip sizes), element-screenshot the port, compare with your own eyes before committing; never flatten to chips/lists; report before shipping if it can't be matched. Chrome (badges, typography column, link-card colors) is site-styled; figures keep source values. (3) **Text fidelity is machine-verified**: scratchpad `fidelity.mjs` extracts every h1/h2/p/li/td/th/label block from the source, strips inline tags to ' ' (not space — `Claude<span>Sense</span>` must stay "ClaudeSense"), decodes entities, and asserts each appears verbatim in the rendered page's innerText at desktop + phone; CSS-uppercased labels match case-insensitively. (4) Wide source figures (980px grids, 880px hero frames) get a breakout column (`max-w-[880..1012px]`) outside `max-w-2xl`; tables scroll horizontally on phone as in the source. (5) Hero images ship byte-identical to `public/images/<station>/`; skill-file views live in `public/skill-files/`. (6) Numbered story labels become an orange number + site h2 (`StoryHeader`); page-local components per file (WONYP precedent).

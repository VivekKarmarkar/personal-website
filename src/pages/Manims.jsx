...
import { manims, subjects } from '../data/manims'

function ManimCard({ item }) {
  return (
    <Link
      to={item.url}
...
export default function Manims() {
  ...
  const filteredManims = selectedSubjects.length === 0
    ? manims
    : manims.filter(item =>
        item.subjects.some(s => selectedSubjects.includes(s))
      )
      ...
      <article className="w-full py-16 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <section className="mb-12">
            <h1 className="text-4xl font-bold mb-2">Manims</h1>
            <p className="text-dark-muted text-lg">
              Mathematical animations for physics and beyond
            </p>
          </section>
          ...
          <div className="flex gap-8">
            ...
            <main className="flex-1">
              {filteredManims.length === 0 ? (
                <div className="text-center py-16 text-dark-muted">
                  <p>No manims available yet. Check back soon!</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredManims.map(item => (
                    <ManimCard key={item.id} item={item} />
                  ))}
                </div>
              )}
            </main>
